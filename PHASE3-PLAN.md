...
|---|---|---|---|---:|
| **W1** ✅ | 沖煮當下體驗（2026-07-15 完成；TIMER-5 專注計時模式待補） | FR-13、FR-14 | TIMER、RCP | ≈ 8 |
...
| TIMER-5 | **（後續，FR-13.5）專注計時模式** | 手機 UX 修正：計時期間改為全螢幕計時畫面（大碼表＋固定不動的悶蒸/分段/結束大按鈕），**計時中不渲染任何輸入框**，僅唯讀分段清單即時滾動；RCP-5 引導同畫面顯示；結束計時才返回表單補水量/手法。修正「每按分段就新增輸入列、畫面撐高、焦點跑掉，得捲回才能再按」的問題。實作要點：抽出 `TimerOverlay`（或以既有 `brew-timer` 的 running 狀態切換為全螢幕 portal/dialog）；沿用現有 `onUpdatePour`/`onAppendPour`/`nextTarget` 邏輯，不動資料寫回。次要方案：sticky 控制列＋計時中收合輸入列（工較小，列為備案）。 | TIMER-1, TIMER-2 | M |
